...
// Debug credentials for testing (remove in production)
...
const LoginForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Debug mode bypasses Supabase authentication
      if (DEBUG_MODE && 
          formData.email === DEBUG_CREDENTIALS.email && 
          formData.password === DEBUG_CREDENTIALS.password) {
        console.log("Debug login successful, bypassing Supabase authentication");
        
        // Store debug session in localStorage
        localStorage.setItem("debug_session", "active");
        
        // Set debug toast notification
        toast({
          title: "Debug Mode",
        ...
        setTimeout(() => {
          navigate("/dashboard?debug=true", { replace: true });
        }, 100);
  ...
  // Helper function to pre-fill debug credentials
  const fillDebugCredentials = () => {
    if (DEBUG_MODE) {
      setFormData({
        ...formData,
        email: DEBUG_CREDENTIALS.email,
        password: DEBUG_CREDENTIALS.password,
      });
    }
  };
    ...
    <div className="space-y-6 animate-fadeIn">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {DEBUG_MODE && !isSignUp && (
          <Button
            type="button"
            variant="outline"
            className="w-full border-yellow-500 text-yellow-700 hover:bg-yellow-100 mb-2"
      ...
      {DEBUG_MODE && !isSignUp && (
        <div className="mt-4 p-2 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-800">
          <p className="font-medium">Debug Credentials:</p>
          <p>Email: {DEBUG_CREDENTIALS.email}</p>
          <p>Password: {DEBUG_CREDENTIALS.password}</p>
        </div>
      )}
    </div>
  );
